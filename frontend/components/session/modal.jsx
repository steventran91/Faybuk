...
class Modal extends React.Component {
  ...
  handleSubmit(e) {
    ...
    if (
      Object.values(this.state).some((val) => val === "") ||
      this.state.birthday === "0/0/0"
    ) {
      let firstErr = true;
      Object.keys(this.state).forEach((key) => {
        if (
          this.state[key] === "" ||
          (key === "birthday" && this.state.birthday === "0/0/0")
        ) {
  ...
  handleRequired(id) {
    ...
    return () => {
      if (
        this.state[id] === "" ||
        (id === "birthday" &&
          this.state[id].split("/").some((el) => el === "0"))
      ) {
  ...
  render() {
    ...
    const errorArr = this.props.errors.signup.length
      ? this.props.errors.signup.map((error) => {
          return <li>{error}</li>;
        })
      : [];
            ...
            <form>
              ...
              <div className="small">
                ...
                <input
                  autoFocus
                  ref={this.first_name}
                  className={this.state.first_name_border ? "required" : ""}
                  onFocus={this.handleFocus("first_name")}
                  onBlur={this.handleRequired("first_name")}
                  type="text"
                ...
                <input
                  ref={this.last_name}
                  className={this.state.last_name_border ? "required" : ""}
                  onFocus={this.handleFocus("last_name")}
                  onBlur={this.handleRequired("last_name")}
                  type="text"
              ...
              <div>
                {this.state.email_error ? (
                  <ErrorTip error={"Email can't be blank"} class={"left"} />
                ) : (
                  ""
                )}
                <input
                  ref={this.email}
                  className={this.state.email_border ? "required" : ""}
                  onFocus={this.handleFocus("email")}
                  onBlur={this.handleRequired("email")}
                  type="text"
              ...
              <div>
                {this.state.password_error ? (
                  <ErrorTip error={"Password can't be blank"} class={"left"} />
                ) : (
                  ""
                )}
                <input
                  ref={this.password}
                  className={this.state.password_border ? "required" : ""}
                  onFocus={this.handleFocus("password")}
                  onBlur={this.handleRequired("password")}
                  type="password"
              ...
              <div className="sub">
                {this.state.birthday_error ? (
                  <ErrorTip error={"Birthday can't be blank"} class={"left"} />
                ) : (
                  ""
                )}
                <label>Birthday</label>
                <select
                  className={this.state.birthday_border ? "required" : ""}
                  onFocus={this.handleFocus("birthday")}
                  onBlur={this.handleRequired("birthday")}
                  onChange={this.handleDate("month")}
              ...
              <div className="sub">
                {this.state.gender_error ? (
                  <ErrorTip error={"Gender can't be blank"} class={"left"} />
                ) : (
                  ""
                )}
                <label>Gender</label>
                <div
                  ...
                  className={
                    this.state.gender_border
                      ? "required gender-container"
                      : "gender-container"
                  }
                >
                  <label>Female</label>
